...
class SearchResultsView(ListView):
    ...
    def get_queryset(self):
        ...
        object_list = Bb.objects.filter(
            Q(title__icontains=query) | Q(content__icontains=query) | Q(writer__icontains=query)
        )
        return object_list
...
def detail(request, rubric_pk, pk):
    ...
    context = {'bb': bb, 'comments': comments, 'form': form}
...
def profile_bb_change(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            bb = form.save()
            messages.add_message(request, messages.SUCCESS, 'Книга исправлена')
            return redirect('main:profile')
    else:
...
def profile_bb_delete(request, pk):
    ...
    if request.method == 'POST':
        bb.delete()
        messages.add_message(request, messages.SUCCESS, 'Книга удалена')
        return redirect('main:profile')
    else:
        context = {'bb': bb}
...
def robot_txt(request):
    return render(request, 'main/robot.txt', {})
